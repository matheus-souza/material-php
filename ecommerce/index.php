...
$app->get('/checkout', function () {
    ...
    if (is_null($address->getdesaddress())) {
        $address->setdesaddress('');
    }
    if (is_null($address->getdesnumber())) {
        $address->setdesnumber('');
    }
    if (is_null($address->getdescomplement())) {
        $address->setdescomplement('');
    }
    if (is_null($address->getdesdistrict())) {
        $address->setdesdistrict('');
    }
    if (is_null($address->getdescity())) {
        $address->setdescity('');
    }
    if (is_null($address->getdesstate())) {
        $address->setdesstate('');
    }
    if (is_null($address->getdescountry())) {
        $address->setdescountry('');
    }
    if (is_null($address->getdeszipcode())) {
...
$app->get('/boleto/{idorder}', function (Request $request) {
    ...
    $dadosboleto["demonstrativo2"] = "Mensalidade referente a nonon nonooon nononon<br>Taxa bancária - R$ " . number_format($taxa_boleto,
            2, ',', '');
